...
console.log("Cleaning public/assets/images...");
...
const boudoirs = fs.readdirSync("./boudoir", { recursive: true });
const photos = fs.readdirSync("./photo", { recursive: true });

boudoirs.forEach((boudoir) => {
  if (!imageBoudoirs.includes(boudoir)) {
    fs.unlinkSync(`./boudoir/${boudoir}`);
  }
});

photos.forEach((photo) => {
  if (!imagePhotos.includes(photo)) {
    fs.unlinkSync(`./photo/${photo}`);
  }
});

console.log("Cleaning public/assets/images... Done");
